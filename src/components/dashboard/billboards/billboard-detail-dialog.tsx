...
export function BillboardDetailDialog({ open, billboardId, onClose }: BillboardDetailDialogProps) {
	...
	React.useEffect(() => {
		...
		(async () => {
			try {
				setLoading(true);
				const d = await getBillboardDetail(billboardId);
				if (!ignore) {
					setData(d);
					setError(null);
				}
	...
	// ⬇️ Extract reviews from transactions safely
	const reviews = React.useMemo(() => {
		return (data?.transaction ?? [])
			...
			.map((r) => ({
				...
				avatar: resolveImgUrl(r.user?.profilePicture),
	...
	const average = reviews.length ? reviews.reduce((s, r) => s + r.stars, 0) / reviews.length : null;

	const ownerUser = React.useMemo(() => {
		// prefer nested owner.user; fallback to ownerUser
		return data?.owner?.user ?? data?.ownerUser ?? null;
	}, [data]);

	const ownerAvatar = resolveImgUrl(ownerUser?.profilePicture);
					...
					<Stack spacing={2}>
							...
							<Stack direction={{ xs: "column", sm: "row" }} spacing={2} alignItems="center">
								...
								<Stack spacing={1} sx={{ flex: 1, width: "100%" }}>
									...
									<Stack direction="row" spacing={1} flexWrap="wrap">
										...
										{data.orientation && (
											<Chip icon={<DirectionsIcon />} size="small" label={`Orientation: ${data.orientation}`} />
										)}
										...
										{data.landOwnership && (
											<Chip icon={<BadgeIcon />} size="small" label={`Land Owner: ${data.landOwnership}`} />
										)}
										{typeof data.score === "number" && <Chip size="small" label={`Score: ${data.score.toFixed(3)}`} />}
									</Stack>
								</Stack>

								{/* Price summary + Owner profile */}
								<Stack spacing={1.25} alignItems={{ xs: "flex-start", sm: "flex-end" }} sx={{ minWidth: 220 }}>
									{/* prices */}
									...
									{ownerUser && (
										...
										>
											<Avatar
												...
												imgProps={{ crossOrigin: "anonymous", referrerPolicy: "no-referrer" }}
											>
												{(ownerUser.username?.[0] ?? "U").toUpperCase()}
											</Avatar>
											<Stack sx={{ minWidth: 0 }}>
												<Typography variant="subtitle2" noWrap title={ownerUser.username}>
													{ownerUser.username}
												</Typography>
												<Typography variant="caption" color="text.secondary" noWrap title={data.owner?.companyName}>
													{data.owner?.companyName ?? "—"}
												</Typography>
												{(ownerUser.email || ownerUser.phone) && (
													<Typography
														variant="caption"
														color="text.secondary"
														noWrap
														title={`${ownerUser.email ?? ""} ${ownerUser.phone ?? ""}`}
													>
														{ownerUser.email ?? ownerUser.phone}
													</Typography>
												)}
						...
						<Paper variant="outlined" sx={{ p: 2 }}>
							<Grid container spacing={2}>
								...
								<Field
									label="Province"
									value={data.city?.province?.name ?? data.provinceName ?? data.provinceId ?? "-"}
								/>
								<Field label="Created" value={safeDate(data.createdAt)} />
								<Field label="Updated" value={safeDate(data.updatedAt)} />
								<Field
										...
										return (
											<ImageListItem key={img.id}>
												...
												<img
													...
													onError={(e) => {
														// optional: keep to avoid broken icon
