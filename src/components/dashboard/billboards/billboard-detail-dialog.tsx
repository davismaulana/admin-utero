...
export function BillboardDetailDialog({ open, billboardId, onClose }: BillboardDetailDialogProps) {
	...
	// image hero from service shape: data.image[]
					...
					<Stack spacing={2}>
							...
							<Stack direction={{ xs: "column", sm: "row" }} spacing={2} alignItems="center">
								...
								<Stack spacing={1} sx={{ flex: 1, width: "100%" }}>
									...
									<Stack direction="row" spacing={1} flexWrap="wrap">
										...
										{data.landOwner && (
											<Chip icon={<BadgeIcon />} size="small" label={`Land Owner: ${data.landOwner}`} />
										)}
									</Stack>
								</Stack>
						...
						<Paper variant="outlined" sx={{ p: 2 }}>
							<Grid container spacing={2}>
								{/* Show readable names if provided, fallback to id */}
						...
						{/* Gallery from `image` (service shape) */}
						<Paper variant="outlined" sx={{ p: 2 }}>
	...
	function statusColor(s?: string | null) {
		// Your service uses "Available" | "Unavailable" | string
		switch (s) {
			case "Available":
				return "success";
			case "Unavailable":
